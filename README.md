...
```
extract_fred >> extract_yahoo >> extract_events >> transform >> load >> analysis >> refresh_dashboard
```
...
### Yahoo Finance
Yahoo Finance provides market price data for gold and for a Treasury bond ETF.

- **Gold Futures** (continuous front-month contract). This is the standard proxy for the spot gold price in US dollars.
- **TLT**: the iShares 20+ Year Treasury Bond ETF. It converts Treasury yield movements into a price series.

### GDELT Project
- **Real-time structured events from global news**
